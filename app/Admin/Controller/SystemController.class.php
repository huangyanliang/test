  ...
  public function sysmod(){
	...
	if($save==''){
		...
		if($data){
		 ...
		 $dobj        = M("systemconfig");
		 $dobj = $dobj->where('Id=1')->save(array('metatitle'=>$metatitle,'icpnote'=>$icpnote,'metades'=>$metades,'metakey'=>$metakey,'companyname'=>$companyname,'address'=>$address,'companyurl'=>$companyurl,'tel'=>$tel,'fax'=>$fax,'email'=>$email,'slogan'=>$slogan,'sys_notice'=>$sys_notice,'sys_code'=>$sys_code,'c_site'=>$c_site,'c_text'=>$c_text,'adminpage'=>$adminpage));
		 if($dobj){
		   $this->success('资料更新成功');
		 }else{
