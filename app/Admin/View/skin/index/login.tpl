...
<body>
<style>
 body{ font-size:12px;margin:0; padding:0; background:url(__img__/adminbg.jpg) no-repeat top center;}
 div{margin:0; padding:0;}
 .login{ width:494px; height:449px; background:url(__img__/login.png) no-repeat center center; margin:12% auto 0px auto; overflow:hidden; }
 ...
 .user_input input{width:277px; height:59px; padding-left:53px; color:#fff;background:url(__img__/login_bg.png) no-repeat 0 -118px ; border:none;font-family:"微软雅黑"; line-height:59px; outline:none;}
 .user_input input:focus{background:url(__img__/login_bg.png) no-repeat 0 -59px ;} 
 .pass_input{ width:330px; height:59px; margin:10px auto 0px auto; overflow:hidden;}
 .pass_input input{width:277px; height:59px; padding-left:53px; color:#fff;background:url(__img__/login_bg.png) no-repeat 0 -177px ; border:none;font-family:"微软雅黑"; line-height:59px; outline:none;}
 .pass_input input:focus{background:url(__img__/login_bg.png) no-repeat 0 0px ;} 
 .code_input{ width:330px; height:59px; margin:10px auto 0px auto; overflow:hidden;}
 .code_input input{width:124px; height:59px; padding-left:53px; color:#fff;background:url(__img__/login_bg.png) no-repeat 0 -236px ; border:none;font-family:"微软雅黑"; float:left; line-height:59px; outline:none;}
 .code_input input:focus{background:url(__img__/login_bg.png) no-repeat 0 -295px ;} 
 .sub_btn{ width:318px; height:48px; margin:15px auto 0px auto; overflow:hidden;}
 .sub_btn input{ background:url(__img__/login_btn.png) no-repeat 0 0; overflow:hidden; height:48px; width:318px; border:none; cursor:pointer;}
...
<body>
<!--[if IE 8]>
 <div class="browser"><img src="__img__/exclamation.png" />&nbsp;您的浏览器版本过低，请升级您的浏览器，建议您升级成ie9,或以上版本。否则部分功能将失效，点击 <a href="http://windows.microsoft.com/zh-cn/internet-explorer/download-ie-MCM?FORM=MI09JG&OCID=MI09JG" target="_blank">去升级</a> 吧</div>
<![endif]-->
<!--[if IE 7]>
 <div class="browser"><img src="__img__/exclamation.png" />&nbsp;您的浏览器版本过低，请升级您的浏览器，建议您升级成ie9,或以上版本。否则部分功能将失效，点击 <a href="http://windows.microsoft.com/zh-cn/internet-explorer/download-ie-MCM?FORM=MI09JG&OCID=MI09JG" target="_blank">去升级</a> 吧</div>
<![endif]-->
...
<div class="login">
 <div class="login_logo"><img src="__img__/login_logo.png" /></div> 
...
<script>
 ...
 function ckbhadmin(){
   ...
   if(td.username.value==''){
     $("#showerror").css("background-image","url(__img__/exclamation.png)").text('请先输入您的后台用户名');
	 td.username.focus();
	 return false;
   }
   if(td.userpass.value==''){
     $("#showerror").css("background-image","url(__img__/exclamation.png)").text('请输入后台登录密码');
	 td.userpass.focus();
	 return false;
   }
   if(td.checkcode.value.length!=5){
     $("#showerror").css("background-image","url(__img__/exclamation.png)").text('请输入后台验证码');
	 td.checkcode.focus();
	 return false;
   }
   //验证中..
   $("#showerror").css("background-image","url(__img__/loadingSmall.gif)").html('<font color="#ffffff">请输入后台验证码</font>');
   ajax({
		method : 'post',
		...
		success : function (data) {
		  ...
		  (loginTips!='') ? $("#showerror").css("background-image","url(__img__/exclamation.png)").text(loginTips) : '';
		},
		async : true
	 });
   return false;
